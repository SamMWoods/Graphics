...
void settings() {
  ...
  myUI.addPlainButton("Draw Polygon", 5, 210);

  myUI.addRadioButton("select", 5, 245, "group1");

  myUI.addPlainButton("Delete", 5, 315);
  ...
  myUI.addRadioButton("Move", 5, 280, "group1");

  myUI.addPlainButton("Quit", 5, 600);

  myUI.addMenu("Image Manipulation", 5, 350, menu1Items);
...
void handleUIEvent(UIEventData eventData) {
  ...
  if ( toolMode.equals("Colour Picker") ) {
    String[] args = {"Colour Picker"};
    ColourPicker sa = new ColourPicker();
    PApplet.runSketch(args, sa);
    toolMode = " ";
  }

  if ( toolMode.equals("Draw Polygon") ) {
    String[] args = {"Polygon"};
    Polygon sa = new Polygon();
    PApplet.runSketch(args, sa);
    toolMode = " ";
  }  
...
public class Polygon extends PApplet {
  ...
  color PolygonColour = color(0, 0, 0);
  ...
  void addPoint() {
    pPoints.add( new PVector(mouseX, mouseY));
    if(PolygonColour == color(0, 0, 0))
    {
      noFill();
    } else {
      fill(PolygonColour);
    ...
    for (PVector newPoints : pPoints) {
      float NewX= newPoints.x;
      float newY =newPoints.y;
      vertex(NewX, newY);
    }
  }
