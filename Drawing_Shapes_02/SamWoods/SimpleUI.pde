...
public class SimpleUI{
   ...
   //
    ...
    //Turn RadioButton Off (black Border)
...
class Menu extends Widget{
  ...
  public Menu(String uiname, String uilabel, int x, int y, String[] menuItems, SimpleUI manager)
    {
    super(uiname,uilabel,x,y,70,30);
    parentManager = manager;
...
class UIRect{
  ...
  boolean isPointInside(PVector p){
    ...
    if(   drawingList.isBetweenInc(p.x, this.left, this.right) && drawingList.isBetweenInc(p.y, this.top, this.bottom) ) return true;
    return false;
  }
